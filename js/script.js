...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Map file names to their correct-answers arrays
  const answerMap = {
    'past-perfect-30-questions.html': [
      'A','C','B','D','A','B','C','A','D','B',
      'C','A','D','B','C','A','D','B','C','A',
      'D','B','C','A','B','C','A','D','B','C'
    ],
    'past-simple-30-questions.html': [
      'C','D','A','B','A','D','B','C','A','B',
      'D','C','A','D','B','C','A','D','B','C',
      'D','A','B','C','A','D','B','C','A','D'
    ],
    'past-perfect-continuous-30-questions.html': [
      'C','B','D','B','C','D','C','B','D','C',
      'C','B','D','B','C','D','C','B','C','D',
      'B','C','D','B','C','D','B','C','D','B'
    ],
    'past-continuous-30-questions.html': [
      'C','B','D','A','B','D','C','B','A','D',
      'C','B','A','D','B','C','D','A','B','D',
      'C','A','B','D','A','D','B','C','A','B'
    ],
    'present-simple-30-questions.html': [
  'B','D','A','C','D','B','C','A','B','D',
  'A','C','B','D','A','C','B','D','C','A',
  'D','B','C','A','D','C','B','A','D','B'
],
'present-perfect-30-questions.html': [
    'C','A','D','B','C','D','A','B','D','C',
    'A','B','D','C','A','D','B','A','C','D',
    'B','C','A','D','B','C','D','A','C','B'
  ]
    ...
    // add more file => array pairs here when you create new quizzes
  };

  // get current HTML file name
  const fileName = window.location.pathname.split('/').pop();

  // select correct answers for this page
  const correctAnswers = answerMap[fileName] || [];

  // Basic checks
  if (!form) {
    console.warn('Quiz form not found (id="quiz-form"). Script stopped.');
    return;
  }
  if (!checkBtn || !resetBtn || !showBtn || !scoreDisplay) {
    console.warn('One or more control elements (#check-btn, #reset-btn, #show-btn, #score) are missing.');
  }
  if (correctAnswers.length === 0) {
    console.warn(`No correctAnswers found for "${fileName}". Make sure the file name matches a key in answerMap.`);
  }
  ...
  function grade() {
    ...
    questions.forEach((qCard, index) => {
      ...
      if (selected && expected && selected.value === expected) {
        ...
        qCard.classList.remove('incorrect');
        score++;
      } else {
        qCard.classList.add('incorrect');
        qCard.classList.remove('correct');
      }
    });

    scoreDisplay.textContent = `Your score: ${score} / ${form.querySelectorAll('.question-card').length}`;
  }

  function revealAnswers() {
    const questions = form.querySelectorAll('.question-card');
    questions.forEach((qCard, index) => {
      let answerReveal = qCard.querySelector('.answer-reveal');
      if (!answerReveal) {
        answerReveal = document.createElement('div');
        answerReveal.className = 'answer-reveal';
        qCard.appendChild(answerReveal);
      }
      answerReveal.textContent = `Correct answer: ${correctAnswers[index] || 'N/A'}`;
    });
  }
  ...
  function resetQuiz() {
    ...
    questions.forEach(qCard => {
      ...
      const answerReveal = qCard.querySelector('.answer-reveal');
      if (answerReveal) answerReveal.textContent = '';
    });
    scoreDisplay.textContent = '';
  }

  checkBtn && checkBtn.addEventListener('click', grade);
  showBtn && showBtn.addEventListener('click', revealAnswers);
